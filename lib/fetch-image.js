...
function fetchImage(imageUrl, opts = {}) {
  ...
  if (!raw) return Promise.reject(err('imageUrl 仅支持 http/https', 400));
      ...
      (res) => {
        ...
        if (status >= 300 && status < 400 && res.headers.location) {
          ...
          if (ctx.redirectsLeft <= 0) {
            fail(Object.assign(err('下载失败：重定向过多', 502), { httpStatus: status }));
            return;
          }
          let next;
          try {
            ...
            fail(Object.assign(err('下载失败：无效重定向', 502), { httpStatus: status }));
            return;
          }
          fetchOnce(next, { ...ctx, redirectsLeft: ctx.redirectsLeft - 1 }).then((result) => {
            if (settled) return;
            settled = true;
            resolve(result);
          }, fail);
        ...
        if (status < 200 || status >= 300) {
          res.resume();
          fail(Object.assign(err(`下载失败，HTTP ${status}`, 502), { httpStatus: status }));
          return;
        }
        ...
        if (Number.isFinite(len) && len > ctx.maxBytes) {
          res.destroy();
          fail(Object.assign(err('图片过大（上限 20MB）', 413), { httpStatus: status }));
          return;
        }
        ...
        res.on('error', (e) => fail(Object.assign(e, { statusCode: 502, httpStatus: status })));
        res.on('end', () => {
          if (settled) return;
          const buffer = Buffer.concat(chunks);
          if (!looksLikeImage(mime, parsed.pathname) && !looksLikeImage(mime, imageUrl)) {
            fail(Object.assign(err('仅支持 jpeg / png / webp 图片', 400), { httpStatus: status }));
            return;
          }
          settled = true;
          resolve({
